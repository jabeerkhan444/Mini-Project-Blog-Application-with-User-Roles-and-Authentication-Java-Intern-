...
<header>
    ...
    <nav>
        <a href="adminPosts">Home</a>
        <a href="createPost.jsp">Create Post</a>
        <c:if test="${not empty sessionScope.user}">
            <a href="LogoutHomeServlet">Logout</a>
        </c:if>
    </nav>
</header>
...
<div class="container">
    <c:if test="${not empty posts}">
        <c:forEach var="post" items="${posts}">
            <div class="post" onclick="window.location.href='updatePost.jsp?id=${post.id}'">
                <c:if test="${not empty post.filePath}">
                    <img src="${pageContext.request.contextPath}/uploads/${post.filePath}" alt="Post Image/Video">
                </c:if>
